...
export function useForumCategories() {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const url = `${API_URL}/software/forum/categories?lang=${locale}`;
        ...
        console.error('Error al obtener categorías de foro:', msg);
        if (isMounted) {
          setError(
            msg ||
              (locale === 'es'
                ? 'No se pudieron cargar las categorías del foro'
