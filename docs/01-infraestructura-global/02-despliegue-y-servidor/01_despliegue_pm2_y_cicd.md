...
server {
    ...
    location ~ ^/(bible/(verses|translations|morphology|books|historical|literary|evangelism)|software/(ai|blog|cybersecurity|forum|news|projects|tutorials|infrastructure)|portfolio/(contact|projects|sandbox)) {
...
Controla el ciclo de vida, resolución determinista de rutas absolutas (`path.resolve`), inyección segura de variables de entorno mediante `loadEnvFile()` con precedencia inmutable (donde `NODE_ENV: 'production'` y puertos/hosts locales siempre sobrescriben los valores de desarrollo que puedan residir en los archivos `.env`) y los límites de memoria de los procesos unificados en el VPS:
* **`backend-nest`:** Inicia `./dist/main.js` desde `backend/` (puerto `3000`, `HOST: '127.0.0.1'`). En producción, Pino opera con streaming JSON estándar de alto rendimiento (`transport: undefined`), activando `pino-pretty` únicamente si `NODE_ENV === 'development'`. Límite de reinicio: `200 MB`.
* **`frontend-next`:** Inicia `./server.js` en modo Next.js Standalone desde `frontend/web/.next/standalone/frontend/web` (puerto `3001`, `HOSTNAME: 'localhost'`). Inyecta variables del servidor (como `GROQ_API_KEY`) desde `frontend/web/.env`. Límite de reinicio: `200 MB`. Consumo real promedio: **~90 MB**.
