'use client';

import React from 'react';
import Image from 'next/image';

export function AuthorSidebarCard() {
    return (
        <div className="p-6 rounded-3xl glass-convex-panel border border-white/5 text-center space-y-4 shadow-xl">
            {/* Avatar circular con relieve cóncavo */}
            <div className="w-20 h-20 mx-auto rounded-full p-1 glass-concave-panel border border-white/10 shadow-inner flex items-center justify-center relative group">
                <div className="w-full h-full rounded-full overflow-hidden relative flex items-center justify-center bg-black/40">
                    <Image
                        src="/software/logo/logo_fondo_circular_color_.png"
                        alt="Jorge Doicela"
                        width={72}
                        height={72}
                        className="object-contain transition-transform duration-300 group-hover:scale-105"
                        unoptimized
                    />
                </div>
            </div>

            {/* Nombre y Título */}
            <div className="space-y-1">
                <h4 className="text-base sm:text-lg font-bold text-[var(--header-title)] tracking-tight">
                    Jorge Doicela
                </h4>
                <p className="text-xs font-mono text-zinc-500 dark:text-zinc-400 font-medium">
                    Especialista en DevSecOps & Cloud
                </p>
            </div>

            {/* Biografía Técnica */}
            <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed font-light">
                Ingeniero enfocado en arquitecturas desacopladas, alta disponibilidad en entornos con recursos acotados y seguridad perimetral.
            </p>

            {/* Enlaces y Redes Sociales */}
            <div className="pt-3 border-t border-black/5 dark:border-white/5">
                <div className="flex items-center justify-center gap-4 text-zinc-400">
                    <a
                        href="https://github.com/JorgeDoicela"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="hover:text-white transition-colors p-1"
                        title="GitHub de Jorge Doicela"
                        aria-label="GitHub"
                    >
                        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true">
                            <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
                        </svg>
                    </a>
                    <a
                        href="https://www.linkedin.com/in/jorgedoicela/"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="hover:text-white transition-colors p-1"
                        title="LinkedIn de Jorge Doicela"
                        aria-label="LinkedIn"
                    >
                        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true">
                            <path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.28 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.75M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z" />
                        </svg>
                    </a>
                    <a
                        href="mailto:jorge.doicela.m@gmail.com"
                        className="hover:text-white transition-colors p-1"
                        title="Enviar correo"
                        aria-label="Email"
                    >
                        <svg className="w-4 h-4 fill-none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    );
}
