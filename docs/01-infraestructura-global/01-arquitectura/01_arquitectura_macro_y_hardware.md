...
Adicionalmente, implementa una **resolución multi-tenant unificada de rutas y assets**. Siguiendo el principio Open/Closed (OCP), el middleware no utiliza listas estáticas de archivos; en su lugar, cualquier recurso solicitado dentro del contexto de un subdominio o del dominio raíz es mapeado automáticamente a su espacio de nombres (`public/<dominio>/`), preservando como excepciones únicamente los assets explícitamente globales (`GLOBAL_ROOT_ASSETS = new Set(['/sw.js'])`), mientras que `/favicon.ico` es reescrito deterministamente hacia el logo circular del subdominio activo (`public/<dominio>/logo/logo_fondo_circular_color_.png`). En producción, Nginx despacha estos archivos directamente desde disco con mapas de `$host` y `$favicon_file` (Zero-RAM), mientras que en desarrollo local o ejecuciones standalone, Next.js garantiza paridad total sin depender de proxies externos ni archivos huérfanos.
