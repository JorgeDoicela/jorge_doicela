...
* **Barra Lateral de Recirculación Editorial ([`FeaturedPostsSidebarCard.tsx`](/software/components/FeaturedPostsSidebarCard.tsx)):**
  ...
  * Disposición editorial asimétrica limpia: cada fila muestra los metadatos contextuales ricos (`categoryMeta` o `tag`, ej. *95 estrellas GitHub*, *Anthropic — MCP_SERVER*, *Aviso HIGH — Ciberseguridad, Linux*) en tipografía mono (`text-[11px] font-mono text-slate-500 dark:text-zinc-400`), titular con transición cromática en hover y miniatura nítida a la derecha (`w-14 h-14 rounded-xl overflow-hidden glass-concave-panel`) exclusivamente cuando el post dispone de imagen de portada real (`coverImage`).
  * Erradica por completo la repetición monótona de fechas idénticas, los iconos SVG decorativos artificiales y los gradientes pastel genéricos en miniaturas ausentes, permitiendo que los artículos sin fotografía expandan su contenido con total alineación visual a la izquierda.
...
* **Directorio Tipográfico con Acordeón por Especialidad ([`ExploreTopicsSidebarCard.tsx`](/software/components/ExploreTopicsSidebarCard.tsx)):**
  * Inspirado en la navegación técnica de **Marcus Hutchins (MalwareTech)** y optimizado para la ergonomía del sidebar fijo (`sticky`), implementa un **acordeón interactivo individual por cada especialidad**:
    ...
    * **Estética de Rama Técnica (Tree / Branch View):** Al expandir una categoría, se despliega un riel vertical con línea guía (`border-l-2 border-blue-500/25`), nodos circulares interactivos con micro-escalado en hover, metadatos contextuales en tipografía mono (`text-[10px] font-mono`) y enlaces directos a cada publicación sin bloques cóncavos oscuros invasivos. Cuenta además con un enlace terminal `Ver todo en {Categoría} (N) ↗` y pastilla cóncava neumórfica (`glass-concave-panel px-2.5 py-0.5 rounded-md text-[11px] font-mono`) con conteo en tiempo real desde el backend NestJS (`/software/hub`).
...
* **Componentes Modulares Especializados (`components/markdown/`):**
   ...
   2. **Bloques de Código de Alta Precisión con Cabecera Inteligente ([`CodeBlock.tsx`](/software/components/markdown/CodeBlock.tsx)):**
     ...
     * Botón interactivo para copiar código con feedback de confirmación (`¡Copiado!`).
  ...
  4. **Paneles de Resumen Técnico y Callouts Blueprint Glass ([`CalloutBlock.tsx`](/software/components/markdown/CalloutBlock.tsx)):**
      * **Panel de Resumen / Arquitectura (`post.architectureOverview`, `post.remediation`):** Marco de cristal con gradiente vítreo sutil (`glass-convex-panel`), titular semántico en tipografía mono técnica en mayúsculas (`text-[11px] font-mono font-bold uppercase tracking-wider`) y tipografía aireada de alto contraste, manteniendo sobriedad absoluta sin puntos pulsantes ni etiquetas redundantes.
     * **Alertas y Directivas Estándar de GitHub:** Soporte automático para `[!NOTE]`, `[!TIP]`, `[!IMPORTANT]`, `[!WARNING]`, `[!CAUTION]` con micro-iconos de Lucide (`Info`, `Lightbulb`, `AlertCircle`, `AlertTriangle`, `ShieldAlert`), borde de color semántico y contenedor vítreo elevado.
     * **Blockquotes Editoriales:** Línea vertical izquierda de riel (`border-l-4 border-blue-500/80`) con fondo vítreo translúcido y texto en cursiva.
